...
- Storage, indexing, and query execution in small relational database systems.
...
`MiniRedBase` is a modern C++ educational relational database under development. Its architecture is inspired by Stanford RedBase, with project organization informed by OceanBase MiniOB.

- Uses C++20, CMake, and cross-platform CI to keep the core implementation small and testable.
- Plans to implement pages, a buffer pool, LRU, heap files, tuples, RIDs, a B+ tree, and a catalog.
- Plans to support basic CRUD through a simplified SQL parser and Volcano execution model.
- Targets 10,000 to 15,000 lines of core code, with either WAL crash recovery or an LRU-K benchmark as an extension.
- Current status: the project scaffold, CLI entry point, and smoke tests are ready; database internals are not implemented yet.
