...
我持续研究一个具体的系统问题：**数据包如何穿过虚拟化主机，以及如何在不破坏安全性和正确性的前提下观察或加速这条路径？** 我主要使用 Linux、eBPF/XDP、C/C++ 和 Go 构建规模可控、能够测试和复现的数据面原型。

我看重清晰的系统边界、可测量的行为、安全的回退路径，以及能够被其他人重新运行和验证的实验。
